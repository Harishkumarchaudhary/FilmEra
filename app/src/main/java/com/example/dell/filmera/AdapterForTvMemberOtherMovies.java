...
public class AdapterForTvMemberOtherMovies extends RecyclerView.Adapter<AdapterForTvMemberOtherMovies.UserViewHoldy> {
    ArrayList<ApiClassForTvMemberOtherShows.MemberOtherMovies> arrayList;
    Context context;

    AdapterForTvMemberOtherMovies(Context context, ArrayList<ApiClassForTvMemberOtherShows.MemberOtherMovies> arrayList) {
    ...
    @NonNull
    @Override
    public AdapterForTvMemberOtherMovies.UserViewHoldy onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        LayoutInflater layoutInflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View view = layoutInflater.inflate(R.layout.top_rated_of1_element_layout, parent, false);
        return new AdapterForTvMemberOtherMovies.UserViewHoldy(view);
    }

    @Override
    public void onBindViewHolder(@NonNull final AdapterForTvMemberOtherMovies.UserViewHoldy holder, final int position) {
        String s = arrayList.get(position).name;
        ...
        holder.image2.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ApiClassForFavourites apiClassForFavourites = new ApiClassForFavourites(arrayList.get(position).id, arrayList.get(position).name, arrayList.get(position).poster_path);
